...
const MyHotels = () => {
  ...
  if (!hotelData) {
    return (
      <div className="container mx-auto text-xl font-semibold italic dark:text-white">
        No Hotel Found
      </div>
    );
  }
  ...
  return (
    <div className=" space-y-5 my-6">
      <div className="flex justify-between">
        <h1 className="text-3xl font-bold dark:text-white">My hotels</h1>
        <Link
          to="/add-hotel"
      ...
      <div className="grid grid-cols-1 gap-6">
        {hotelData &&
          hotelData.length > 0 &&
          hotelData.map((hotel) => (
            <div className="flex flex-col justify-between border border-slate-300 rounded-lg p-6 gap-3 dark:text-white dark:bg-slate-500">
              <h2 className="text-xl font-semibold">{hotel.name}</h2>
              <div className="text-base font-normal whitespace-pre-line">
                {hotel.description}
              </div>
              <div className="grid grid-cols-5 gap-2 text-sm text-slate-800 dark:text-white flex-wrap">
